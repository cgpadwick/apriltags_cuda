...
class TensorRTInference {
 public:
  ...
  bool infer(const float* input, float* output) {
    ...
    CHECK_CUDA(cudaMemcpyAsync(mDeviceBuffers[0], input, mInputSize,
    ...
    CHECK_CUDA(cudaMemcpyAsync(output, mDeviceBuffers[1], mOutputSize,
...
void printCameraSettings(const cv::VideoCapture& cap) {
  ...
  std::cout << "  " << frame_width << "x" << frame_height << " @" << frame_rate
            << "FPS" << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  while (!camera_started) {
    try {
      ...
      if (cap.isOpened()) {
        camera_started = true;
        std::cout << "Camera started successfully on index " << 0 << std::endl;
      } else {
  ...
  // cv::imwrite("bgr.png", bgr_img);
  /*cv::imshow("Image", bgr_img);
  cv::waitKey(0);*/

  bgr_img = cv::imread("data/note.png", cv::IMREAD_COLOR);
  // Preprocess the data before putting it through the engine.
  cv::cvtColor(bgr_img, rgb_img, cv::COLOR_BGR2RGB);
  cv::resize(rgb_img, rgb_img_subsampled, cv::Size(h, w), 0, 0,
             cv::INTER_LINEAR);
  // cv::imwrite("rgb_img_subsampled.png", rgb_img_subsampled);
  ...
  for (int c = 0; c < 3; c++) {
    ...
    std::memcpy(input_buffer + channel_offset, channel_data,
                h * w * sizeof(float));
  }

  // Prepare output buffer
  std::vector<float> output(inference.getOutputSize() / sizeof(float));
  std::cout << "outputsize: " << output.size() << std::endl;
  ...
  analyze_detections(output, 0.90);
